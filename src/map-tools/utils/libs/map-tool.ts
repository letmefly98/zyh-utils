...
level
 ...
 13     0010 0xxx xxxx xxxx  xxxx xxxx xxxx xxxx
  ...
  static readonly MortonTable256 = [
    0x0000,
    0x0001,
    0x0004,
    0x0005,
  ...
  public static level2distance(x: number) {
    return 180 / (1 << x)
  }

  public static wgs2tileid(x: number, y: number, level: number) {
    return MapTool.nds2tileid(MapTool.lon2nds(x), MapTool.lat2nds(y), level)
  }

  public static tileid2wgs(tileid: number): [number, number] {
    const coord = MapTool.tileid2nds(tileid)
    const level = MapTool.tileid2level(tileid)
    return [MapTool.nds2lon(coord[0]), level === 0 ? -90 : MapTool.nds2lat(coord[1])]
  }

  public static tileid2nds(tileid: number) {
    let morton = MapTool.tileid2morton(tileid)
    let lon = 0
    let lat = 0
    for (let i = 0; i < 8; ++i) {
      const split = MapTool.MortonTable16[morton & 0xF]
      lon |= split[0] << (2 * i)
      lat |= split[1] << (2 * i)
      morton >>= 4
    }
    return [MapTool.uint32(lon << 16), MapTool.uint32(lat << 16)]
  }

  public static tileid2morton(tileid: number) {
    const level = MapTool.tileid2level(tileid)
    return (tileid & ((1 << (2 * level + 1)) - 1)) << (2 * (15 - level))
  }

  public static nds2lon(x: number) {
    if (x >= 0x80000000) x -= 0x100000000
    return (90 * x) / 0x40000000
  }

  public static nds2lat(x: number) {
    if (x >= 0x40000000) x -= 0x80000000
    return (90 * x) / 0x40000000
  }

  public static tileid2level(x: number) {
    return Math.floor(Math.log(x) * Math.LOG2E) - 16
  }

  public static nds2tileid(x: number, y: number, level: number) {
    return MapTool.uint32((1 << (level + 16)) | MapTool.nds2tile(x, y, level))
  }

  public static nds2tile(x: number, y: number, level: number) {
    return MapTool.morton2tile(MapTool.nds2morton(x >> 16, y >> 16), level)
  }
   ...
   */
  public static nds2morton(x: number, y: number) {
    return MapTool.uint32(
      MapTool.MortonTable256[x & 0xFF]
      | (MapTool.MortonTable256[y & 0xFF] << 1)
      | (MapTool.MortonTable256[(x >> 8) & 0xFF] << 16)
      | (MapTool.MortonTable256[(y >> 8) & 0xFF] << 17),
    )
  }
  ...
  public static morton2tile(x: number, level: number) {
    const shift = 2 * (15 - level)
    return shift >= 0 && shift < 32 ? x >>> shift : x
  }
   ...
   */
  public static lon2nds(x: number) {
    const y = MapTool.trunc((x / 90) * 0x40000000)
   ...
   */
  public static lat2nds(x: number) {
    const y = MapTool.trunc((x / 90) * 0x40000000)
    return y >= 0 ? y : y + 0x80000000
  }

  public static trunc(x: number) {
