...
describe('normal.mjs', () => {
  ...
  it('random', () => {
    const checkData1 = Array.from({ length: 100 }).fill().map(() => random(5, 8))
    const check1 = data => data.every(n => n > 5 && n < 8)
    const checkData2 = Array.from({ length: 100 }).fill().map(() => random(2))
    const check2 = data => data.every(n => n > 0 && n < 2)
    const checkData3 = Array.from({ length: 100 }).fill().map(() => random(1, -2))
